      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 pb-3 border-b border-gray-200 dark:border-border/20">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {role.is_system ? (
              <span className="text-[9px] font-bold px-1.5 py-0.3 rounded bg-amber-500/10 text-amber-500 border border-amber-500/15">系统内置</span>
            ) : (
              <span className="text-[9px] font-bold px-1.5 py-0.3 rounded bg-blue-500/10 text-blue-500 border border-blue-500/15">自定义</span>
            )}
            <span className="text-[10px] text-gray-500 dark:text-gray-500 font-mono">#{role.code}</span>
          </div>
          {role.description && (
      ...
      {role.permission_codes.length === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mr-1">已授权</span>
            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-accent-blue bg-accent-blue/8 border border-accent-blue/15 px-1.5 py-0.5 rounded">
              {role.permission_codes.length} 项
            </span>
            <span className="text-gray-300 dark:text-gray-600">·</span>
            <span className="text-[10px] text-gray-500 dark:text-gray-400">{modKeys.length} 个模块</span>
          </div>
              ...
              <div key={mod} className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0 flex flex-wrap gap-1">
                  {grouped[mod].codes.map(code => {
                    ...
                    return (
                      <span key={code} className="inline-flex items-center px-2 py-0.5 rounded bg-gray-100 dark:bg-accent-blue/8 text-gray-700 dark:text-blue-300 text-[10px] font-medium border border-gray-200 dark:border-accent-blue/12">
                        {p?.name || code}
                      </span>
                    )
                  })}
