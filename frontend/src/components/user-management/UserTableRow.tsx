...
export function UserTableRow({
  ...
  return (
    <tr className={`hover:bg-gray-50/50 dark:hover:bg-bg-hover/20 transition-colors ${selected ? 'bg-accent-blue/5' : ''}`}>
      ...
      <td className="px-5 py-4">
        {u.department_name ? (
          <span className="inline-flex items-center text-[10px] px-2 py-0.5 rounded-md bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 font-medium border border-blue-200 dark:border-blue-500/20 shadow-sm whitespace-nowrap">
            {u.department_name}
          </span>
        ) : (
      ...
      <td className="px-5 py-4">
        {u.is_admin ? (
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 font-bold border border-amber-200 dark:border-amber-500/20 shadow-sm whitespace-nowrap">
            <Crown size={11} className="text-amber-500" /> 管理员
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-md bg-gray-50 dark:bg-gray-500/10 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-border/20 whitespace-nowrap">
            普通成员
          </span>
        )}
      </td>
      ...
      <td className="px-5 py-4">
        <div className="flex items-center gap-1.5 flex-wrap">
          {u.is_admin ? (
            <span className="inline-flex items-center text-[9px] px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 shadow-sm font-bold whitespace-nowrap" title="管理员天然拥有全套自管与共享 AI 模型管理及调用特权">
              全部 AI 特权
            </span>
          ) : (
            <>
              {u.can_manage_models && (
                <span className="inline-flex items-center text-[9px] px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 shadow-sm whitespace-nowrap" title="自主配置专有模型 API key">
                  自管模型
                </span>
              )}
              {u.use_shared_models && (
                <span className="inline-flex items-center text-[9px] px-1.5 py-0.5 rounded bg-purple-50 dark:bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-500/20 shadow-sm whitespace-nowrap" title="可调用管理员预配置的平台公共共享模型">
                  共享模型
                </span>
              )}
      ...
      <td className="px-5 py-4 text-center">
        <div className="inline-flex flex-col items-center gap-1">
          {u.status === 'resigned' ? (
            <span className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-500/20 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-border/20 font-semibold whitespace-nowrap">
              <UserX size={10} /> 已离职
            </span>
          ) : u.status === 'disabled' ? (
            <button
              onClick={onToggleActive}
              className="inline-flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full cursor-pointer transition-all font-semibold shadow-sm bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 border border-red-200 dark:border-red-500/15 whitespace-nowrap"
          ...
          {isLocked && (
            <span className="inline-flex items-center gap-0.5 text-[9px] px-1.5 py-0.2 rounded bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30 font-mono font-bold whitespace-nowrap" title={`因错误密码尝试锁定至 ${formatTime(u.locked_until)}`}>
              <Lock size={9} /> 锁定制
            </span>
          )}
        </div>
