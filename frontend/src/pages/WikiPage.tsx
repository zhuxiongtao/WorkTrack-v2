...
import {
  ...
  Sparkles, Send, ArrowRightLeft, CornerDownLeft
} from 'lucide-react'
...
export default function WikiPage() {
  ...
  const [allGroupsList, setAllGroupsList] = useState<GroupListItem[]>([])
  ...
  const isDirty = editingPage !== null && (editorTitle !== savedRef.current.title || editorContent !== savedRef.current.content)
  ...
  const loadCollaboratorsAndUsers = async () => {
    ...
    try {
      const [perms, users, groups] = await Promise.all([
        ...
        API.get<UserListItem[]>('/api/v1/users/simple'),
        API.get<GroupListItem[]>('/api/v1/wiki/groups')
      ])
      setCollaborators(Array.isArray(perms) ? perms : [])
      setAllUsersList(Array.isArray(users) ? users : [])
      setAllGroupsList(Array.isArray(groups) ? groups : [])
    } catch {
  ...
  const handleCopyShareLink = () => {
    ...
    const shareUrl = `${window.location.protocol}//${window.location.host}/wiki/public/${selectedSpace.id}/${editingPage.id}?scope=${shareScope}`
  ...
  const handleAddCollaborator = async () => {
    if (!editingPage || !selectedSubjectId) return
    try {
  ...
  }, [handleTextSelection, canEdit, isEditingMode])
  ...
  const handleAiAction = async (action: string) => {
    ...
    if (action === 'translate') { actionLabel = '智能翻译'; prompt = `请帮我将以下选中的内容翻译成地道的英文（English）。直接输出翻译后的纯英文内容，不要有任何中文解释或前言：\n"${selectedText}"` }
    ...
    return (
      <div className="space-y-6 pb-12 animate-fadeIn text-gray-800 dark:text-gray-100">
        ...
        {!loading && spaces.length === 0 && (
          <div className="text-center py-20 rounded-2xl bg-bg-card border border-gray-200 dark:border-border/30 shadow-sm">
            <FolderOpen size={48} className="mx-auto text-gray-400 dark:text-gray-600 mb-3 opacity-40" />
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">暂无任何知识库/文档空间</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5">立即创建一个，解锁在线级的深度编辑和 AI 协同体验吧</p>
          </div>
        )}
        ...
        {(() => {
          ...
          const renderCard = (space: WikiSpace) => {
            ...
            return (
              ...
              >
                {/*书籍封面质感容器 */}
                <div className="relative aspect-[4/5] rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-bg-card border border-gray-200 dark:border-border/10">
                  {/* 全覆盖封面渐变 */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${cover.gradient} opacity-90`} />
                  
                  {/* 书脊质感渐变 */}
                  <div className="absolute left-0 top-0 bottom-0 w-2.5 bg-gradient-to-r from-black/25 via-black/10 to-transparent" />
                  
                  {/* 底部渐变遮罩 — 确保文字在任何封面颜色上都可读 */}
                  <div className="absolute bottom-0 left-0 right-0 h-3/4 bg-gradient-to-t from-black/60 via-black/20 to-transparent pointer-events-none" />
                  
                  {/* 封面内容 */}
                  <div className="relative h-full flex flex-col p-4 text-white-force">
                    {/* 顶部图标 */}
                    <BookOpen size={18} className="mb-2.5 text-white/70" />
                    
                    {/* 空间名称 — 白色粗体 */}
                    <h3 className="text-base font-extrabold leading-snug line-clamp-3 flex-1 text-white-force">
                      {space.name}
                    </h3>
                    
                    {/* 描述 */}
                    {space.description && (
                      <p className="text-xs line-clamp-1 mt-1.5 text-white/80">
                        {space.description}
                      </p>
                    )}
                    
                    {/* 底部信息栏 */}
                    <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-white/15 text-white/75 text-[10px]">
                      {space.is_owner === false ? (
                        space.is_page_collaborative ? (
                          <span className="px-1.5 py-0.5 rounded bg-blue-500/35 text-white-force font-bold flex items-center gap-0.5" title="该空间的某些单页文档已授权给您协助编辑">
                            <FileText size={8} /> 协作文档
                          </span>
                        ) : (
                          <span className="px-1.5 py-0.5 rounded bg-purple-500/35 text-white-force font-bold flex items-center gap-0.5" title="整个在线文档库（整个空间）已被共享给您">
                            <Users size={8} /> 共享空间
                          </span>
                        )
                      ) : space.is_public ? (
                        <span className="px-1.5 py-0.5 rounded bg-white/20 text-white-force font-bold flex items-center gap-0.5">
                          <Globe size={8} /> 公开外链
                        </span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded bg-black/15 text-white/80 font-bold flex items-center gap-0.5">
                          <Lock size={8} /> 私域空间
                        </span>
                      )}
                      <span className="font-mono text-white-force opacity-90">
                  ...
                  <div className="absolute top-1.5 right-1.5 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-black/40 hover:bg-black/60 transition-colors text-white-force"
                      title="编辑空间基本信息"
                    >
                      <Edit3 size={12} />
                    </button>
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-black/40 hover:bg-red-600/80 transition-colors text-white-force"
                      title="注销空间"
                    >
                      <Trash2 size={12} />
                    </button>
            ...
            <div className="space-y-6">
              ...
              {mySpaces.length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
            ...
            <div className="bg-bg-card rounded-2xl p-6 w-full max-w-sm border border-gray-200 dark:border-border/40 shadow-2xl animate-scaleIn" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={handleDeleteSpace}
                  className="px-4 py-2 text-xs bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 shadow-sm cursor-pointer"
                >确认彻底删除</button>
              </div>
            </div>
          </div>
        ...
        <div className="w-56 lg:w-64 flex-shrink-0 border-r border-gray-200 dark:border-border bg-bg-sidebar flex flex-col">
          ...
          <div className="p-3 border-b border-gray-200 dark:border-border bg-gray-50/50 dark:bg-transparent">
            <button
              ...
              className="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 hover:text-gray-800 dark:hover:text-gray-300 mb-2.5 transition-colors cursor-pointer"
            >
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
            {treeLoading ? (
              ...
              <div className="text-center py-12 text-gray-400">
                <p className="text-[10px] italic">空间无在线文档</p>
                <button
                  onClick={() => { setPageForm({ title: '', parent_id: null }); setShowPageDialog(true) }}
                  className="mt-2 text-[10px] text-accent-blue hover:underline font-bold"
                >
                  创建首篇文档
                </button>
              </div>
          ...
          <>
               ...
               <div className="flex items-center gap-1.5 flex-shrink-0 select-none">
                ...
                {isEditingMode ? (
                  <button
                    ...
                    className="px-3 py-1.5 bg-accent-blue hover:bg-blue-600 text-white text-xs font-bold rounded-xl flex items-center gap-1 cursor-pointer shadow-sm transition-colors"
                  >
                    <Check size={12} />
                    <span>完成</span>
                  </button>
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex-1 flex min-h-0 relative">
                ...
                <div 
                  className="flex-1 overflow-y-auto bg-gray-50/50 dark:bg-[#101520]/40 px-4 pt-4 pb-1 md:px-6 md:pt-6 md:pb-1 lg:px-8 lg:pt-8 lg:pb-1" 
                ...
                >
                  ...
                  <div className="max-w-4xl mx-auto bg-white dark:bg-bg-card border border-transparent dark:border-border/30 rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.05)] transition-all duration-300 p-6 md:p-8 lg:p-12 min-h-[calc(100vh-80px)] md:min-h-[calc(100vh-100px)] flex flex-col relative">
                    ...
                    <div className="mb-6 border-b border-gray-150 dark:border-border/15 pb-4 select-none">
                      {isEditingMode ? (
                        <input
                          type="text"
                          value={editorTitle}
                          ...
                          className="w-full text-3xl font-extrabold bg-transparent border-transparent px-0 outline-none focus:ring-0 focus:border-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 font-sans"
                          placeholder="请输入文档标题"
                        />
                      ) : (
                ...
                inlineAiActive ? (
                  ...
                  >
                    ...
                    <div className="text-xs leading-relaxed max-h-48 overflow-y-auto pr-1">
                      {inlineAiLoading ? (
                        <div className="flex items-center gap-2 text-purple-400 font-bold py-6 justify-center animate-pulse">
                    ...
                    {!inlineAiLoading && inlineAiResult && (
                      <div className="flex gap-2 border-t border-white/10 pt-2.5 justify-end select-none">
                        <button
                          onClick={() => { setInlineAiActive(false); setShowAiToolbar(false) }}
                          className="px-2.5 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white text-[10px] font-bold cursor-pointer transition-colors border border-white/10"
                        >
                          放弃
                        </button>
                        <button
                          onClick={() => handleInsertBelowInline(inlineAiResult)}
                          className="px-2.5 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/35 text-emerald-400 border border-emerald-500/25 text-[10px] font-bold cursor-pointer transition-colors"
                          title="在您选中的这行段落正下方，插入 AI 生成的新正文"
                        >
                          ↵ 插入下方
                        </button>
                        <button
                          onClick={() => handleReplaceSelectionInline(inlineAiResult)}
                          className="px-2.5 py-1.5 rounded-lg bg-accent-blue hover:bg-blue-600 text-white text-[10px] font-bold cursor-pointer transition-colors shadow-sm"
                          title="直接用 AI 润色后的结果，覆盖并替换您的当前选中文字"
                        >
                          🔄 覆盖替换
                        </button>
                      </div>
                  ...
                  /* 初始 5 个便捷 AI 工具按钮栏 */
                  <div
                    className="fixed z-[100] flex items-center gap-0.5 bg-gray-950/95 dark:bg-bg-card/95 text-white p-1 rounded-xl shadow-xl border border-gray-800 dark:border-border/40 backdrop-blur-xl ai-prevent-deselect animate-scaleIn"
                    style={{
                      left: `${aiToolbarCoords.x}px`,
                      top: `${aiToolbarCoords.y}px`,
                      transform: 'translate(-50%, -100%)',
                  ...
                  >
                    <button onClick={() => handleAiAction('polish')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-white/15 text-[11px] font-bold cursor-pointer transition-colors" title="精修润色">
                      <Sparkles size={11} className="text-purple-400" />
                      润色
                    </button>
                    <button onClick={() => handleAiAction('summarize')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-white/15 text-[11px] font-bold cursor-pointer transition-colors" title="核心提炼">
                      总结
                    </button>
                    <button onClick={() => handleAiAction('translate')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-white/15 text-[11px] font-bold cursor-pointer transition-colors" title="翻译为地道英文">
                      翻译
                    </button>
                    <button onClick={() => handleAiAction('extend')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-white/15 text-[11px] font-bold cursor-pointer transition-colors" title="逻辑续写">
                      续写
                    </button>
                    <button onClick={() => handleAiAction('explain')} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-white/15 text-[11px] font-bold cursor-pointer transition-colors" title="解释概念">
                      解释
                    </button>
                  </div>
                )
              ...
              {aiSidebarOpen && (
                <div className="w-80 lg:w-96 flex-shrink-0 border-l border-gray-200 dark:border-border bg-bg-sidebar flex flex-col h-full ai-prevent-deselect animate-slideLeft">
                  ...
                  <div className="p-4 border-b border-gray-200 dark:border-border bg-gray-50/50 dark:bg-transparent flex items-center justify-between select-none shrink-0">
                    ...
                    <button onClick={() => { setAiSidebarOpen(false); setAiHistory([]) }} className="p-1 rounded hover:bg-gray-200 dark:hover:bg-bg-card text-gray-450 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 cursor-pointer transition-colors"><X size={14} /></button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4.5">
                    {aiHistory.length === 0 ? (
                      <div className="text-center py-20 text-gray-400 select-none">
                        ...
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1.5 max-w-[80%] mx-auto leading-relaxed">
                      ...
                      aiHistory.map((h, i) => (
                        <div key={i} className={`flex flex-col ${h.role === 'user' ? 'items-end' : 'items-start'}`}>
                          <span className="text-[10px] text-gray-400 mb-1 font-semibold select-none">{h.role === 'user' ? '我的指令' : 'AI 协作草稿'}</span>
                          <div className={`p-3.5 rounded-2xl text-xs leading-relaxed max-w-[90%] border shadow-sm ${
                            ...
                            {h.role === 'assistant' && (
                              <div className="mt-3.5 pt-2.5 border-t border-gray-100 dark:border-border/10 flex items-center gap-2 select-none">
                                <button
                                  onClick={() => handleReplaceSelection(h.content)}
                                  className="px-2.5 py-1.5 rounded bg-accent-blue/10 hover:bg-accent-blue/20 text-accent-blue border border-accent-blue/15 text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-all"
                                ...
                                <button
                                  ...
                                  className="px-2.5 py-1.5 rounded bg-gray-100 hover:bg-gray-200 dark:bg-bg-hover dark:hover:bg-bg-card border border-gray-200 dark:border-border/40 text-gray-600 dark:text-gray-300 text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-all"
                    ...
                    {aiLoading && (
                      <div className="flex flex-col items-start animate-pulse">
                        <span className="text-[10px] text-gray-400 mb-1">AI 写作智囊正在深度思考编写中...</span>
                  ...
                  <div className="border-t border-gray-150 dark:border-border bg-gray-50/50 dark:bg-bg-card/30 p-3.5 flex flex-col gap-2 shrink-0">
                    {selectedText && (
                      <div className="px-3 py-1.5 bg-purple-50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-500/15 rounded-xl text-[10px] text-purple-600 dark:text-purple-400 font-bold flex items-center gap-1.5 select-none animate-slideDown max-w-full truncate shadow-sm">
                        <Sparkles size={10} className="text-purple-500 animate-pulse shrink-0" />
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-3.5 pb-5 border-b border-gray-150 dark:border-border/15">
                ...
                <div className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50 dark:bg-bg-hover/10 border border-gray-150 dark:border-border/20 transition-all hover:bg-gray-100/30 dark:hover:bg-bg-hover/20">
                  <div className="min-w-0 pr-4">
                    ...
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1 leading-relaxed">
                      启用后，整空间将可以通过特定的文档链接向互联网公开，任何访客在无需登录的情况下均具有该文档只读（Viewer）权限。
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {isSpacePublic && (
                  <div className="space-y-4.5 bg-gray-50/50 dark:bg-bg-hover/5 p-4 rounded-xl border border-gray-150 dark:border-border/10 animate-slideDown">
                    {/* 分享访问范围选择 */}
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 mb-1.5">共享访问范围</label>
                      <select
                        value={shareScope}
                    ...
                    <div className="grid grid-cols-2 gap-3.5 border-t border-gray-150 dark:border-border/10 pt-3">
                      ...
                      {expireMode === 'custom' ? (
                        <div className="animate-slideDown">
                          <label className="block text-[10px] font-bold text-gray-500 mb-1.5 flex items-center gap-1"><Clock size={11} /> 共享到期时间</label>
                          <input
                            type="datetime-local"
                        ...
                        <div className="animate-slideDown">
                          <label className="block text-[10px] font-bold text-gray-500 mb-1.5 flex items-center gap-1"><Clock size={11} /> 有效期状态</label>
                    ...
                    <div className="pt-2 border-t border-gray-200 dark:border-border/10 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-gray-700 dark:text-gray-300 flex items-center gap-1"><Key size={11} /> 启用共享提取密码/提取码</span>
                        <button
                          type="button"
                          onClick={async () => {
                            const nextUsePass = !usePassword
                            setUsePassword(nextUsePass)
                            await syncShareConfig(isSpacePublic, nextUsePass, sharePassword, expireMode, shareExpiresAt)
                          }}
                      ...
                      {usePassword && (
                        <div className="flex gap-2 items-center animate-slideDown">
                          ...
                          <button
                            ...
                            className="p-1 px-2.5 h-[28px] text-[10px] bg-bg-hover hover:bg-gray-200 dark:hover:bg-bg-card border border-gray-200 dark:border-border/40 text-gray-500 hover:text-gray-800 dark:text-gray-400 rounded-lg cursor-pointer font-bold"
                          >
                            随机生成
                          </button>
                        </div>
              ...
              <div className="space-y-3.5 pb-5 border-b border-gray-150 dark:border-border/15">
                <span className="text-[11px] font-bold tracking-wider text-gray-400 dark:text-gray-500 uppercase flex items-center gap-1.5">
                  ...
                  新增协作者 (Add Collaborator)
                </span>

                <div className="bg-gray-50/50 dark:bg-bg-hover/5 p-4 rounded-xl border border-gray-150 dark:border-border/10 space-y-4 shadow-sm animate-slideDown">
                  {/* Row 1: Type + Target Search */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-1.5">协作者类别</label>
                      <select
                        value={selectedSubjectType}
                        onChange={e => {
                          setSelectedSubjectType(e.target.value as any)
                          setSelectedSubjectId(null)
                        }}
                        style={{ colorScheme: theme }}
                        className="w-full h-10 px-3 rounded-xl bg-white dark:bg-bg-input border border-gray-200 dark:border-border/60 text-xs text-gray-700 dark:text-gray-200 outline-none focus:border-accent-blue focus:ring-2 focus:ring-accent-blue/15 cursor-pointer font-bold"
                      >
                        <option value="user" className="bg-white dark:bg-bg-input text-gray-800 dark:text-gray-200">系统用户</option>
                        <option value="group" className="bg-white dark:bg-bg-input text-gray-800 dark:text-gray-200">用户组</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-1.5">搜索并选择</label>
                      <SearchableSelect
                        options={selectedSubjectType === 'user'
                          ? allUsersList.map(u => ({ id: u.id, label: u.name || u.username, sub: `@${u.username}` }))
                          : allGroupsList.map(g => ({ id: g.id, label: g.name }))
                        }
                        value={selectedSubjectId || ''}
                        onChange={(val) => setSelectedSubjectId(val as number)}
                        placeholder={selectedSubjectType === 'user' ? '选择用户...' : '选择用户组...'}
                  ...
                  {/* Row 2: Role Selection + Submit Button */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 border-t border-gray-100 dark:border-border/10 pt-3">
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-1.5">赋予角色</label>
                      <select
                        ...
                        className="w-full h-10 px-3 rounded-xl bg-white dark:bg-bg-input border border-gray-200 dark:border-border/60 text-xs text-gray-700 dark:text-gray-200 outline-none focus:border-accent-blue focus:ring-2 focus:ring-accent-blue/15 cursor-pointer font-bold"
                      >
                        <option value="viewer" className="bg-white dark:bg-bg-input text-gray-800 dark:text-gray-200">查看者 (只读)</option>
                        <option value="editor" className="bg-white dark:bg-bg-input text-gray-800 dark:text-gray-200">编辑者 (可写)</option>
                        <option value="admin" className="bg-white dark:bg-bg-input text-gray-800 dark:text-gray-200">管理员 (统筹)</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-transparent mb-1.5">添加</label>
                      <button
                        onClick={handleAddCollaborator}
                        disabled={!selectedSubjectId}
                        className="w-full h-10 bg-accent-blue text-white text-xs font-bold rounded-xl hover:bg-blue-600 disabled:opacity-40 flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-sm"
                      >
                        <Plus size={14} />
                        添加并授权协作者
                      </button>
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                  {loadingCollaborators ? (
                    <div className="text-center py-6 text-gray-400 flex items-center justify-center gap-1.5">
                      ...
                      <span className="text-[11px]">正在刷新权限名单...</span>
                    </div>
                  ) : collaborators.length === 0 ? (
                    <p className="text-xs text-gray-400 dark:text-gray-500 italic py-4 text-center">暂无任何协作者加入，当前仅空间 Owner 具有读写编辑权</p>
                  ) : (
                    collaborators.map(p => {
                      // 查找名称
                      let displayName = `ID: ${p.subject_id}`
                      let details = p.subject_type === 'user' ? '单个用户' : '用户组'
                      if (p.subject_type === 'user') {
                        const u = allUsersList.find(uu => uu.id === p.subject_id)
                        if (u) {
                          displayName = u.name || u.username
                          details = `@${u.username}`
                        }
                      } else {
                        const g = allGroupsList.find(gg => gg.id === p.subject_id)
                        ...
                        <div key={p.id} className="flex items-center justify-between p-3.5 rounded-xl bg-gray-50 dark:bg-bg-hover/15 border border-gray-150 dark:border-border/20 text-xs">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <span className="text-[10px] text-gray-400 dark:text-gray-500 font-mono block mt-0.5">{details}</span>
                            </div>
                          </div>
