...
def create_payment(body: PaymentCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_session)):
    ...
    db.commit()
    db.refresh(p)
    write_log("info", "payment", f"用户 {current_user.username} 新建付款申请 #{p.id}（{p.title}）", db=db)
    nm = _name_map(db, [p.user_id])
    cm = _contract_map(db, [p.contract_id] if p.contract_id else [])
    return _to_out(p, nm, cm)
...
@router.post("/{payment_id}/submit-approval")
def submit_payment_approval(payment_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_session)):
    """提交付款审批：部门负责人/分管领导 → 财务初审 → 总经理 → 出纳付款。"""
    p = db.get(PaymentRequest, payment_id)
    if not p:
        raise HTTPException(404, "付款申请不存在")
    if p.user_id != current_user.id and not current_user.is_admin:
        raise HTTPException(403, "无权操作该付款申请")
    if approval_engine.get_active_instance("payment", payment_id, db):
        raise HTTPException(400, "该付款申请已有进行中的审批")
    try:
        inst = approval_engine.start_approval(
            "payment", payment_id, p, f"付款申请《{p.title}》", current_user, db,
        )
    ...
    if inst.status == "pending":
        ...
        db.commit()
    db.refresh(p)
    write_log("info", "payment", f"付款申请 #{payment_id} 提交审批（实例 #{inst.id}）", db=db)
