...
class Project(SQLModel, table=True):
    ...
    name: str  # 项目名称
    opportunity_amount: Optional[float] = None  # 商机金额（按币种单位）
    deal_amount: Optional[float] = None  # 成交价格（按币种单位）
    ...
    tech_support_person: Optional[str] = None  # 技术支持（选项管理）
    status: str = ""  # 状态（选项管理）
    ...
    cost_amount: Optional[float] = None  # 内部成本金额（按币种单位，cost = 供应商通道费合计）
